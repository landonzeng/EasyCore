//------------------------------------------------------------------------------
// <auto-generated>
//     此代码由工具生成。
//     运行时版本:4.0.30319.42000
//     Website: http://www.freesql.net
//     对此文件的更改可能会导致不正确的行为，并且如果
//     重新生成代码，这些更改将会丢失。
// </auto-generated>
//------------------------------------------------------------------------------
using System;
using System.Collections;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Threading.Tasks;
using Newtonsoft.Json;
using FreeSql.DataAnnotations;
using System.ComponentModel;

namespace WebApi.Module {

	[JsonObject(MemberSerialization.OptIn)]
	public class Tbiz_ShopInfoLog {

		[JsonProperty, Column(IsPrimary = true, IsIdentity = true)]
		public long Id { get; set; }

		/// <summary>
		/// 批次号
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(50)")]
		public string BatchNum { get; set; } = string.Empty;

		/// <summary>
		/// 地址
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(500)")]
		public string Caddress { get; set; } = string.Empty;

		/// <summary>
		/// 描述
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(500)")]
		public string Cdescription { get; set; } = string.Empty;

		/// <summary>
		/// 国家地区
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(50)")]
		public string Country { get; set; } = string.Empty;

		/// <summary>
		/// 实体店ID
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(50)")]
		public string CphyStoId { get; set; } = string.Empty;

		[JsonProperty]
		public DateTime? CreateDate { get; set; }

		/// <summary>
		/// 店面状态
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(50)")]
		public string CshopStation { get; set; } = string.Empty;

		/// <summary>
		/// 简短描述
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(50)")]
		public string CsimDescr { get; set; } = string.Empty;

		/// <summary>
		/// 开店日期
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(50)")]
		public string CstrShopDt { get; set; } = string.Empty;

		/// <summary>
		/// 生效日期
		/// </summary>
		[JsonProperty, Column(DbType = "date")]
		public DateTime? Effdt { get; set; }

		/// <summary>
		/// 集合 ID
		/// </summary>
		[JsonProperty, Column(DbType = "nvarchar(50)")]
		public string SetId { get; set; } = string.Empty;

	}

}
